...
\begin{itemize}
  ...
  \item Exceptionally, if a car is left at more than 3 KM from the nearest power grid station or with more than 80\% of the battery empty, its state is set to "OUT OF SERVICE".  An operator will then charge it on-site and notify the system to set its state to "FREE" again. % todo assumptions because the assignment is not clear
\end{itemize}
...
\begin{figure}[!ht]
  \centering
  \vspace{0.2cm}
  \includegraphics[width=0.6\textwidth]{/RASD/System_Functions/use_car_mockup}\\
  \vspace{0.4cm}
